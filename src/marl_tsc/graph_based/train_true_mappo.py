...
Training entry point for true Graph MAPPO.
...
Differences from train_graph_mappo.py
--------------------------------------
- Uses build_default_graph_mappo_policy (centralised critic) instead of
  build_default_graph_policy (per-agent critic heads).
- Reads global_state_dim from env and passes it to the policy builder.
- Uses TrueMAPPOTrainer instead of GraphMAPPOTrainer.
"""
...
def train_true_mappo(
    ...
    """
    Train a true graph-based MAPPO policy.
    ...
    critic_hidden_dim : int
        Hidden dimension for the centralised critic MLP.
    ...
    try:
        ...
        global_state_dim = env.global_state_dim     # num_agents * obs_dim
        ...
        model, history, model_path = run_training(
            ...
            model_path=str(
                Path(output_dir) / "models" / "true_mappo.pt"
            ),
        )
